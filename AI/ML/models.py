...
# type: ignore
"""
SMURF HUNTER - AML Detection using GraphSAGE
...
def train_model(
    ...
    test_acc, test_prec = evaluate(model, data, data.test_mask, torch_device)
    precision_at_k = compute_precision_at_k(model, data, data.test_mask, torch_device, k_values=[20, 50, 100, 200])
...
if __name__ == "__main__":
    ...
    model_path = os.path.join(os.path.dirname(__file__), 'smurf_hunter_model.pt')
